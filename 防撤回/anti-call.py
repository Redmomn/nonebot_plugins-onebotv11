...
add_grouplist = on_command('开启防撤回', aliases = {'添加防撤回', 'enable'}, permission = SUPERUSER, block = True, priority=2)
...
del_grouplist = on_command('关闭防撤回', aliases = {'删除防撤回', 'disable'}, permission = SUPERUSER, block = True, priority=2)
...
check_grouplist = on_command('查看防撤回群聊', permission = SUPERUSER, block = True,priority=2)
...
add_group = on_command('开启当前防撤回', aliases = {'添加当前防撤回', 'enable here', '开启本群防撤回', '添加本群防撤回'}, permission = SUPERUSER | GROUP_OWNER, block=True, priority=2)
...
del_group = on_command('关闭当前防撤回', aliases = {'删除当前防撤回', 'disable here', '关闭本群防撤回', '删除本群防撤回'}, permission = SUPERUSER | GROUP_OWNER, block=True, priority=2)
...
enable_bypass_admin = on_command('开启绕过管理层', aliases = {'bypass here'}, permission = SUPERUSER | GROUP_OWNER, block = True, priority=2)
...
disable_bypass_admin = on_command('关闭绕过管理层', aliases = {'no bypass here'}, permission = SUPERUSER | GROUP_OWNER, block = True, priority=2)
...
reset_enablelist = on_command('重置开启名单', aliases = {'清空开启名单', 'clear list'}, permission = SUPERUSER, block = True, priority=2)
        ...
        if str(gid) in enablelist['grouplist']:
            ...
            if testfor_bypass_admin_list(str(gid)) and bypass_admin_list[str(gid)]:
                if (info['role'] == 'member') and uid == oid:
                    try:
                        msg = f_msg + response['message']
                        await bot.send_group_msg(group_id = gid, message = msg, auto_escape = False)
                    except Exception:
                        await bot.send_group_msg(group_id = gid, message = MessageSegment.at(uid) + Message('撤回了以下消息：\n'), auto_escape = False)
                ...
                try:
                    ...
                    await bot.send_group_msg(group_id = gid, message = MessageSegment.at(uid) + Message('撤回了以下消息：\n'), auto_escape = False)
                    await bot.send_group_msg(group_id = gid, message = response['message'], auto_escape = False)
                # if response['message'][0]['type'] == 'text':
                #     raw_text = response['message'][0]['data']['text']
                #     await bot.send_group_msg(group_id = gid, message = MessageSegment.at(uid) + Message(f'撤回了以下消息：\n{raw_text}'), auto_escape = False)
                # else:
                #     await bot.send_group_msg(group_id = gid, message = MessageSegment.at(uid) + Message('撤回了以下消息：\n'), auto_escape = False)
                #     await bot.send_group_msg(group_id = gid, message = response['message'], auto_escape = False)
                if str(gid) in private_msg_list.keys():
...
use_help = on_command('防撤回管理', aliases = {'防撤回菜单', 'antirecall menu'}, priority=2, block = True)
